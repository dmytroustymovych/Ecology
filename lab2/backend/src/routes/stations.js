...
router.get('/', async (req, res) => {
  try {
    ...
    const sort = { city_name: 1, station_name: 1 };

    let query = Station.find(filter).sort(sort);
    if (parsedLimit) {
      query = query.limit(parsedLimit).skip((parsedPage - 1) * parsedLimit);
    }

    const stations = await query;
    const total = await Station.countDocuments(filter);
    const pages = parsedLimit ? Math.ceil(total / parsedLimit) : 1;
